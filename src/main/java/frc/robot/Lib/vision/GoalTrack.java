...
public class GoalTrack {
    ...
    public synchronized boolean tryUpdate(double timestamp, Pose2d new_observation) {
        ...
        if (distance < TechConstants.kMaxTrackerDistance) {
    ...
    synchronized void pruneByTime() {
        double delete_before = Timer.getFPGATimestamp() - TechConstants.kMaxGoalTrackAge;
    ...
    synchronized void smooth() {
        if (isAlive()) {
            ...
            for (Map.Entry<Double, Pose2d> entry : mObservedPositions.entrySet()) {
                if (t_now - entry.getKey() > TechConstants.kMaxGoalTrackSmoothingTime) {
                    continue;
                }
                ++num_samples;
    ...
    public synchronized double getStability() {
        return Math.min(1.0, mObservedPositions.size() / (TechConstants.kCameraFrameRate * TechConstants.kMaxGoalTrackAge));
    }
